...
**Technically:** golem is a thin wrapper over [`expr-lang/expr`](https://github.com/expr-lang/expr). The **primary interface is Python**, generated by [`gopy`](https://github.com/go-python/gopy) over a Go core and shipped as prebuilt wheels; the Go core remains directly embeddable by Go services. It compiles each expression once into an immutable, thread-safe `*vm.Program`, caches it in a bounded `hashicorp/golang-lru/v2`, and adds the production layer expr leaves to the caller: a 7-type error API (`errors.As`-switchable), strict `expr.Env` type-checking so an undeclared identifier is a **compile-time** `UndefinedVariableError`, a `safeRun` panic→error boundary, an AST-node cost budget, and a goroutine-raced eval timeout. The Python binding crosses dynamic data as a type-tagged JSON envelope (sidestepping gopy [#254](https://github.com/go-python/gopy/issues/254)) — so there is no second evaluator and no parity drift.
...
The Python package — golem's primary interface — binds the **same Go core** via `gopy`. There is no second evaluator and no parity drift: a float `2.0` authored in a Python notebook evaluates identically when a Go service embeds the core on its hot path.
...
Prebuilt binary wheels (manylinux / macOS x86+arm / Windows) are produced by CI via `gopy` + `cibuildwheel`, so `pip install` needs **no Go toolchain and no C compiler** — the compiled Go engine is baked into the wheel. See [Using golem from Python](#using-golem-from-python-the-primary-interface) for the contract and limitations.
...
The Python interface is native-fast *and* trivially installable, but pays a gopy + JSON-marshalling cost per call — fine, because Python is the authoring/product layer; a Go service that needs the absolute floor on latency embeds the core directly.
...
Python is how most people use golem: `pip install enchanter-golem`, `import golem`, define a formula, evaluate it. Under the hood the Python package binds the **same Go core** that a Go service would embed — there is no second evaluator and no parity drift. It is generated by `gopy` (not a hand-rolled C ABI); gopy emits a CPython extension with GC-safe, handle-based marshalling, and CI ships it as prebuilt wheels so there is **no Go toolchain to install**. The Go core remains the in-process engine doing the work — it is simply invisible from Python.
...
flowchart TD
    PyApp["Python app (primary)"] -->|"gopy binding"| ENV["JSON envelope<br/>(EvalJSON / NewEngineJSON)"]
    ENV --> API
