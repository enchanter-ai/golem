...
The expression engine for data-scientist-authored logic that runs millions of times per second.

**One Go core. One Python skin. One source of truth. Zero silent zeros.**

> "Evaluate `revenue * margin - fixed_cost` across 2M ad auctions/sec."
>
> golem compiled it once against the declared schema, cached the bytecode program,
> and ran it from 12 goroutines at low microseconds per eval. A data scientist
> fat-fingered `margn` in a sibling rule — golem failed it at **compile time** with
> *did you mean "margin"?* instead of silently returning 0 into a live bid. The same
> expression evaluates identically from the Python authoring notebook and the Go RTB
> hot path.
>
...
**In plain English:** Your computed-variable expression returns `0` in production and it makes no sense. golem turns a typo'd variable into a *compile error*, runs the expression in compiled Go at roughly a microsecond, and hands Python and Go the exact same answer.

**Technically:** golem is a thin wrapper over [`expr-lang/expr`](https://github.com/expr-lang/expr). It compiles each expression once into an immutable, thread-safe `*vm.Program`, caches it in a bounded `hashicorp/golang-lru/v2`, and adds the production layer expr leaves to the caller: a 7-type error API (`errors.As`-switchable), strict `expr.Env` type-checking so an undeclared identifier is a **compile-time** `UndefinedVariableError`, a `safeRun` panic→error boundary, an AST-node cost budget, and a goroutine-raced eval timeout. A Python binding generated by [`gopy`](https://github.com/go-python/gopy) over the *same* core crosses dynamic data as a type-tagged JSON envelope (sidestepping gopy [#254](https://github.com/go-python/gopy/issues/254)) — so there is no second evaluator and no parity drift.
...
- Platform teams exposing **expression authoring to data scientists / analysts** who must then run those expressions millions of times/sec on a hot path — adtech/RTB, feature stores, pricing/risk rules, computed metrics, ETL transforms.
- Teams who want a typo to fail at **compile time**, not silently become `0` in a live system.
- Codebases that need **identical evaluation semantics from both Go and Python** out of one engine.
...
- [Python binding (tier-2, best-effort)](#python-binding-tier-2-best-effort)
...
### It gives Go and Python the same answer

The Python package binds the **same Go core** via `gopy`. There is no second evaluator and no parity drift: a float `2.0` authored in a Python notebook evaluates identically on the Go RTB hot path.
...
### Go (production surface)

```sh
go get github.com/enchanter-ai/golem
```

Requires **Go 1.22+** (the floor required by `expr-lang/expr` v1.17.x).

### Python binding (tier-2, best-effort)

```sh
pip install enchanter-golem   # the import stays `import golem`
```

Prebuilt binary wheels (manylinux / macOS x86+arm / Windows) are produced by `cibuildwheel`, so `pip install` needs **no Go toolchain and no C compiler**. See [Python binding](#python-binding-tier-2-best-effort) for the contract and limitations.
...
Booleans, ternaries, strings, null-coalescing — all provided by expr:

```go
e.Eval(`status == "active" and score > 0.8 ? "promote" : "hold"`, vars)
e.Eval(`user?.email ?? "no-email"`, vars)
```

Custom host functions and the curated math stdlib:
...
From Python — the same core, the same answer:
...
| Python binding (same core) | yes — `gopy` over one core | none | separate CEL-python impl | none |
...
The Python binding is native-fast *and* trivially installable, but pays a gopy + JSON-marshalling cost per call — fine, because Python is the authoring/product layer, not the RTB hot path.

## Python binding (tier-2, best-effort)

<a href="../../actions/workflows/wheels.yml"><img alt="Wheels" src="https://img.shields.io/badge/wheels-cibuildwheel-58a6ff"></a>

The Python package binds the **same Go core** — no second evaluator, no parity drift. It is generated by `gopy` (not a hand-rolled C ABI); gopy emits a CPython extension with GC-safe, handle-based marshalling.
...
**How dynamic variables cross the boundary.** gopy marshals `map[string]interface{}` poorly, and its `(value, error)` → Python-exception mapping is buggy for `(string, error)` ([gopy#254](https://github.com/go-python/gopy/issues/254)). So golem never returns a Go `error` across the boundary for the dynamic path; dynamic vars cross as a **JSON string** via `EvalJSON`, which returns a single self-describing envelope:
...
The thin Python wrapper parses the envelope, restores the Python type from the `type` tag (so a float `2.0` stays a Python `float` even though JSON renders it `2`), and raises the mapped exception when `ok=false`. The symmetric `NewEngineJSON(optionsJSON)` carries the engine config (schema, strict flag, cache, cost, timeout) across the same way. gopy still binds the `Engine`/`Program` handles directly; only the dynamic path uses the envelope.

**Limitations (v1):** no Python callables as custom functions (Go-registered only; v2 roadmap); no collection results (slice/map → `EvalError`); NaN/Inf → `EvalError`; a gopy+JSON cost per call. gopy-generated files are not committed — a Makefile target + `wheels.yml` run `gopy build`; only `python/golem/__init__.py`, `pyproject.toml`, and `python/tests/` are checked in.
...
| [`gopy`](https://github.com/go-python/gopy) (build-time) | BSD-3-Clause | code generator; not linked into the Go core |
| [`cibuildwheel`](https://github.com/pypa/cibuildwheel) (build-time) | BSD-2-Clause | wheel builder |

golem itself is **MIT** (see [LICENSE](LICENSE)).
...
- **[gopy](https://github.com/go-python/gopy)** + **[cibuildwheel](https://github.com/pypa/cibuildwheel)** — the Python binding + wheel pipeline.
