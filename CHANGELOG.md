...
- **Python binding (tier-2, best-effort)** — generated by `gopy` over the same Go
  core. Dynamic var maps cross the boundary as a JSON envelope via `EvalJSON`
  (works around gopy [#254](https://github.com/go-python/gopy/issues/254));
  the Python wrapper restores the typed value and raises the mapped exception.
  Wheels built with `cibuildwheel` so `pip install enchanter-golem` needs no Go/C
  toolchain.
- **CI** — `ci.yml` (build / vet / test / `-race` / golangci-lint / staticcheck /
  govulncheck / coverage) and `wheels.yml` (gopy + cibuildwheel matrix + a
